...
Quat4f Quat4f::log() const {
    float len = sqrt(m_elements[1] * m_elements[1] + m_elements[2] * m_elements[2] + m_elements[3] * m_elements[3]);
...
Quat4f Quat4f::exp() const {
    float theta = sqrt(m_elements[1] * m_elements[1] + m_elements[2] * m_elements[2] + m_elements[3] * m_elements[3]);
...
// static
Quat4f Quat4f::lerp(const Quat4f &q0, const Quat4f &q1, float alpha) { return ((q0 + alpha * (q1 - q0)).normalized()); }
...
Quat4f operator*(float f, const Quat4f &q) { return Quat4f(f * q.w(), f * q.x(), f * q.y(), f * q.z()); }

Quat4f operator*(const Quat4f &q, float f) { return Quat4f(f * q.w(), f * q.x(), f * q.y(), f * q.z()); }
