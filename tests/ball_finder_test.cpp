...
TEST(BallFinder, BasicTests) {
    for (int i = 0; i < 100; i++) {
        ...
        bf.AddBall(&b3);
        int num_balls = 0;
        bf.FindAndResizeBalls(center + 0.3 * rng.RandNormalizedVector(), [&](Ball *b) {
            num_balls += 1;
            return -1;
        });
        ASSERT_EQ(num_balls, 2);
        bf.FindAndResizeBalls(center + Vector3f(4.9, 0, 0), [&](Ball *b) {
            num_balls += 1;
            return -1;
        });
        ASSERT_EQ(num_balls, 3);
        bf.FindAndResizeBalls(center + Vector3f(4.9, 1, 0), [&](Ball *b) {
            num_balls += 1;
            return -1;
        });
        ASSERT_EQ(num_balls, 3);
        bf.FindAndResizeBalls(center + Vector3f(4, 1, 0), [&](Ball *b) {
            num_balls += 1;
            return 0.01;
        });
        ASSERT_EQ(num_balls, 4);
        bf.FindAndResizeBalls(center + Vector3f(3, 0, 0.009), [&](Ball *b) {
            num_balls += 1;
            return -1;
        });
